...
方差计划：β_start = 0.0002, β_end = 0.04

第1步添加的高斯噪声的方差是0.0002, 第2步添加的高斯噪声的方差是0.0002 + (0.04 - 0.0002) / 1000。
...
```ad-danger
title: 采样算法（去噪算法，反向扩散算法）

1. $\mathbf{x}_T\sim\mathcal{N}(\mathbf{0},\mathbf{I})$ # 从高斯分布中采样一张噪声图片
2. **for** $t=T,...,1$ **do**
...
def add_noise(self, x_0, t):
    ...
    # 返回第t步的图像x_t和添加的噪声noise，后面作为训练数据
    return x_t, noise
```
...
\sin \left( \frac{t}{10000^{\frac{i}{D}}} \right), \quad i\text{为偶数时}\\ \\
\cos \left( \frac{t}{10000^{\frac{i}{D}}} \right), \quad i\text{为奇数时}
\end{cases}
$$
