...
```
<im_start>system: xxxxx<im_end>
<im_start>user: xxxxx<im_end>
<im_start>assistant: xxxxx<im_end>

<image_start>image_token_1, image_token_2, ...<image_end>
<video_start>video_token, .....<video_end>
<audi_start>audio_token, .....<audio_end>
<text_start>word token....<text_end>
```
...
![[图片token和文本token.excalidraw|1000]]
## 3. 准备训练数据集
...
sft：
  - 训练时，输入是“问题和回答”，预测目标是“回答”。只计算“回答”部分的损失。
  - 推理时，输入是“问题”。

图生文：
  - 训练时，输入是“图片token列表和图片文本token列表”，预测目标是“图片文本”。只计算“图片文本”部分的损失。
  - 推理时，输入是“10个图片token”
```
