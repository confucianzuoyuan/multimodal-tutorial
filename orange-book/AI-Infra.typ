...
除了线程块外，具备9.0及以上计算能力的GPU还有一种称为簇的可选分组层级。簇是一组线程块，像线程块和网格一样，可以布局为一维、二维或三维。 图 5 展示了线程块网格，也被组织成簇。指定簇不会改变网格中的网格尺寸或线程块的索引。

#figure(
  image("ai-infra-figures/grid-of-clusters.png"),
  caption: [当指定簇时，线程块在网格中的位置相同，但在包含簇中也有一定位置。],
)

指定簇会将相邻的线程块分组成簇，并在集群层面提供一些额外的同步和通信机会。具体来说，集群中的所有线程块都在一个GPC中执行。 图 6 展示了在GPC中指定集群时，线程块如何调度到SM。由于线程块同时调度且在同一GPC内，不同区块内但同一簇内的线程可以通过合作组提供的软件接口相互通信和同步。簇中的线程可以访问集群中所有块的共享内存，这被称为分布式共享内存。簇的最大大小取决于硬件，且不同设备之间有所不同。

图 6 展示了簇内线程块如何同时调度在GPC内的SM上。簇内的线程块始终在网格内相邻。

#figure(
  image("ai-infra-figures/thread-block-scheduling-with-clusters.png"),
  caption: [当指定簇时，簇中的线程块按其簇形状在网格中排列。簇的线程块会同时调度在单个GPC的SM上。],
)

==== Warp和SIMT

在一个线程块内，线程被组织成32个线程组成的组，称为Warp。Warp在单指令多线程（SIMT）范式中执行内核代码。在SIMT中，Warp中的所有线程执行的是相同的核代码，但每个线程可能遵循代码中的不同分支。也就是说，虽然程序中的所有线程执行相同的代码，但线程不必遵循相同的执行路径。

当线程被Warp执行时，它们会被分配一条Warp通道。Warp通道编号为0到31，线程块中的线程以可预测的方式分配给Warp，详见硬件多线程。

Warp中的所有线程同时执行同一条指令。如果Warp中的某些线程执行时遵循控制流分支，而其他线程不遵循，则不遵循分支的线程将被遮蔽，而执行该分支的线程则被执行。例如，如果条件句只对Warp中一半的线程成立，那么在激活线程执行这些指令时，Warp的另一半会被遮蔽。这一情况如图 7 所示。当Warp中不同的线程遵循不同的代码路径时，这有时称为Warp发散。因此，当Warp内的线程沿同一控制流路径时，GPU的利用率会被最大化。

#figure(
  image("ai-infra-figures/active-warp-lanes.png"),
  caption: [在这个例子中，只有线程索引为偶数的线程执行`if`语句的正文，其他线程在执行正文时被掩码。],
)

在SIMT模型中，Warp中的所有线程都同步通过内核。硬件执行可能有所不同。有关独立线程执行的部分，关于这一区别的重要性，请参见相关章节。不鼓励利用Warp执行如何映射到真实硬件的知识。CUDA编程模型和SIMT表示，Warp中的所有线程都会一起推进代码。只要遵循编程模型，硬件可以以对程序透明的方式优化掩码通道。如果程序违反了该模型，可能会导致不同GPU硬件中存在未定义的行为。

虽然编写CUDA代码时不必考虑Warp，但理解Warp执行模型有助于理解全局内存整合和共享内存库访问模式等概念。一些高级编程技术利用线程块内的Warp专用化，以限制线程发散并最大化利用率。这种优化和其他优化利用了线程在执行时被分组为Warp的知识。

Warp执行的一个含义是，线程块最好指定为线程总数，即32的倍数。使用任意数量的线程是合法的，但当总数不是32的倍数时，线程块的最后一个Warp区会有一些执行过程中未使用的通道。这很可能导致该Warp的功能单元利用率和内存访问不优。

#tip[
  SIMT常被拿来与单指令多数据（SIMD）并行处理比较，但它们存在一些重要区别。在SIMD中，执行遵循单一的控制流路径，而在SIMT中，每个线程允许遵循自己的控制流路径。因此，SIMT不像SIMD那样有固定的数据宽度。关于SIMT的更详细讨论可见于SIMT执行模型 。
]

=== GPU内存

在现代计算系统中，高效利用内存与最大化运算功能单元的使用同样重要。异构系统拥有多个内存空间，GPU除了缓存外，还包含各种可编程的片上内存。以下章节将更详细介绍这些内存空间。

==== 异构系统中的DRAM存储器

GPU和CPU都直接连接DRAM芯片。在拥有多个GPU的系统中，每个GPU都有自己的内存。从设备代码的角度来看，连接到GPU的DRAM被称为全局内存，因为它对GPU中的所有SM都可访问。这些术语并不意味着它在系统内的任何地方都能访问。连接到CPU的DRAM称为系统内存或主机内存。

像CPU一样，GPU使用虚拟内存寻址。在所有当前支持的系统中，CPU和GPU使用统一的虚拟内存空间。这意味着系统中每个GPU的虚拟内存地址范围都是唯一且与CPU及系统中其他所有GPU不同的。对于给定的虚拟内存地址，可以确定该地址是在GPU内存中还是系统内存中，并且在拥有多个GPU的系统中，可以确定哪个GPU内存包含该地址。

有CUDA API用于分配GPU内存、CPU内存，并在CPU与GPU之间、GPU内部或多GPU系统中的GPU之间复制分配。数据的局部性可以在需要时被显式控制。下文将讨论的统一内存允许CUDA运行时或系统硬件自动处理内存的放置。

==== GPU中的片上存储器

除了全局内存外，每个GPU还配备了一些片上内存。每个SM都有自己的寄存器文件和共享内存。这些内存是SM的一部分，可以从内存内执行的线程极快地访问，但其他SM中运行的线程无法访问。

寄存器文件存储线程局部变量，这些变量通常由编译器分配。共享内存对线程块或簇内的所有线程均可访问。共享内存可用于线程块或簇的线程之间的数据交换。

寄存器文件和SM中的统一数据缓存具有有限大小。关于存储器寄存器文件大小、统一数据缓存以及如何配置L1和共享内存平衡，可在"按计算能力存储信息"中查找。寄存器文件、共享内存空间和L1缓存在线程块内的所有线程之间共享。

要将线程块调度到SM，每个线程所需的寄存器总数乘以线程块中的线程数，必须小于或等于SM中可用的寄存器数。如果线程块所需的寄存器数量超过寄存器文件大小，核无法启动，必须减少线程块中的线程数以使线程块可启动。

共享内存分配是在线程块层面进行的。也就是说，与按线程分配的寄存器分配不同，共享内存的分配是整个线程块共有的。

===== 高速缓存

除了可编程存储器外，GPU还拥有L1和L2缓存。每个SM都有一个L1缓存，它是统一数据缓存的一部分。一个更大的L2缓存由GPU内所有SM共享。这可以在图 2 的GPU方块图中看到。每个SM还有一个独立的常量缓存 ，用于缓存在全局内存中被声明为核生命周期内常数的值。编译器也可以将内核参数放入常量内存。这可以通过允许内核参数独立于L1数据缓存缓存，提升核性能。

==== 统一内存

当应用程序在GPU或CPU上显式分配内存时，该内存仅对运行在该设备上的代码访问。也就是说，CPU内存只能从CPU代码访问，GPU内存只能从运行在GPU上的内核访问。CUDA API用于在CPU和GPU之间复制内存，在正确的时间显式地将数据复制到正确的内存。

CUDA的一个功能称为统一内存，允许应用程序进行内存分配，这些内存可从CPU或GPU访问。CUDA运行时或底层硬件在需要时允许访问或将数据迁移到正确位置。即使是统一内存，也通过尽量减少内存迁移，并尽可能访问直接连接到内存的处理器的数据来实现最佳性能。

系统的硬件特性决定了内存空间之间数据的访问和交换方式。统一内存部分介绍了统一内存系统的不同类别。统一内存部分包含了关于统一内存在所有情况下使用和行为的更多细节。

== CUDA平台

NVIDIA CUDA平台包含许多软件和硬件，以及许多为实现异构系统计算而开发的重要技术。本章介绍了CUDA平台中一些对应用开发者需要理解的重要基本概念和组件。本章与编程模型一样，并不针对任何编程语言，而是适用于所有使用CUDA平台的语言。

=== 计算能力与流式多处理器版本

每块NVIDIA GPU都有一个计算能力（CC）数，表示该GPU支持哪些功能，并指定该GPU的一些硬件参数。这些规范载于第5.1节附录中。所有NVIDIA GPU及其计算能力的列表可在CUDA GPU计算能力页面维护。

计算能力以格式`X.Y`中的主版本号和次要版本号表示，其中`X`为主要版本号，`Y`为次要版本号。例如，CC 12.0有一个主要版本是12，一个次要版本是0。计算能力直接对应SM的版本号。例如，CC 12.0 GPU中的SM版本是`sm_120`。该版本用于标记二进制。

第5.1.1节展示了如何查询并确定系统中GPU的计算能力。

=== CUDA工具包与NVIDIA驱动

NVIDIA驱动可以被视为GPU的操作系统。NVIDIA驱动是一个软件组件，必须安装在主机系统的操作系统上，是所有GPU用途（包括显示和图形功能）所必需的。NVIDIA驱动是CUDA平台的基础。除了CUDA，NVIDIA驱动还提供了所有其他使用GPU的方法，例如Vulkan和Direct3D。NVIDIA驱动有版本号，例如r580。

CUDA工具包是一套库、头文件和工具，用于编写、构建和分析利用GPU计算的软件。CUDA工具包是独立于NVIDIA驱动的软件产品。

CUDA运行时是CUDA工具包提供的库的一个特殊案例。CUDA运行时既提供API也有一些语言扩展，用于处理常见任务，如内存分配、GPU与其他GPU或CPU之间的数据复制，以及启动内核。CUDA运行时的API组件称为CUDA运行时API。

CUDA兼容性文档详细介绍了不同GPU、NVIDIA驱动和CUDA工具包版本之间的兼容性。

=== CUDA运行时API和CUDA驱动API

CUDA运行时API是在一个名为CUDA驱动API的低层API之上实现的，后者是由NVIDIA驱动暴露的API。本指南重点介绍CUDA运行时API所提供的API。如果需要，所有相同的功能仅使用驱动API即可实现。部分功能仅通过驱动API提供。应用程序可以只使用其中一种API或两者都使用。CUDA驱动API涵盖运行时与驱动API之间的互相操作的特性。

CUDA运行时API函数的完整API参考可在CUDA运行时API文档中找到。

=== 并行线程执行（PTX）

CUDA平台的一个基本但有时不可见的层是并行线程执行（PTX）虚拟指令集架构（ISA）。PTX是一种面向NVIDIA GPU的高级汇编语言。PTX在真实GPU硬件的物理指令集架构上提供了抽象层。像其他平台一样，应用程序也可以直接用这种汇编语言编写，尽管这样做会增加软件开发的复杂性和难度。

领域专用语言（DSL）和高级语言编译器可以生成PTX代码作为中间表示（IR），然后使用NVIDIA的离线或即时编译（JIT）工具生成可执行的二进制GPU代码。这使得CUDA平台能够使用其它语言来编程，而不仅仅是只能使用NVIDIA提供的工具（如NVCC：NVIDIA CUDA编译器）支持的语言。

由于GPU能力随时间变化和增长，PTX虚拟ISA规范有版本限制。PTX版本与SM版本一样，对应计算能力。例如，支持计算能力8.0所有功能的PTX被称为`compute_80`。

关于PTX的完整文档可在PTX ISA中找到。

=== Cubin和Fatbin

CUDA应用程序和库通常用更高阶语言如C++编写。该高级语言被编译成PTX，然后将PTX编译成物理GPU的真实二进制，称为CUDA二进制，简称cubin。Cubin有针对特定SM版本（如`sm_120`）的二进制格式。

使用GPU计算的可执行文件和库二进制文件包含CPU和GPU代码。GPU代码存储在一个称为fatbin的容器中。fatbin可以包含针对多个不同目标的Cubin和PTX。例如，一个应用程序可以用针对多个不同GPU架构的二进制文件构建，即不同的SM版本。当应用程序运行时，其GPU代码会加载到特定GPU上，并从fatbin中使用该GPU的最佳二进制文件。

#figure(
  image("ai-infra-figures/fatbin.png"),
  caption: [可执行文件或库的二进制文件包含CPU二进制代码和GPU代码的fatbin容器。fatbin可以包含cubin GPU二进制代码和PTX虚拟ISA代码。PTX代码可以为未来目标编译JIT。],
) <fatbin>

Fatbin还可以包含一个或多个PTX版本的GPU代码，具体用途详见PTX兼容性。@fatbin 展示了一个应用程序或库二进制的示例，其中包含多个GPU代码库版本以及一个PTX代码版本。

==== 二进制兼容性

NVIDIA GPU在某些情况下保证二进制兼容性。具体来说，在主要版本的计算能力中，具有大于或相当于目标Cubin版本的次要计算能力的GPU可以加载并执行该cubin。例如，如果一个应用程序包含一个为计算能力8.6编译代码的cubin，那么该cubin可以在计算能力为8.6或8.9的GPU上加载和执行。然而，它无法加载到计算能力为8.0的GPU，因为GPU的CC小版本号0低于代码的次要版本号6。

NVIDIA GPU在主要计算能力版本之间不兼容二进制。也就是说，为计算能力8.6编译的cubin代码不会加载到计算能力9.0的GPU。

在讨论二进制代码时，二进制代码通常被称为有版本，如上述示例中的`sm_86`。这就像说二进制是为计算能力8.6而建的一样。这种简写常用来说明开发者为NVIDIA CUDA编译器nvcc指定该二进制构建目标。

==== PTX兼容性

GPU代码可以以二进制或PTX形式存储在可执行文件中，这些内容在Cubin和Fatbin中都有覆盖。当应用程序存储PTX版本的GPU代码时，该PTX可以在应用运行时为任何计算能力进行JIT编译，满足或更高的计算能力。例如，如果某个应用程序包含`PTX for compute_80`，该PTX代码可以通过JIT编译到后续的SM版本，比如在应用运行时`sm_120`。这使得未来GPU实现了前向兼容，无需重建应用程序或库。

==== JIT（即时编译）

应用程序在运行时加载的PTX代码会被设备驱动程序编译为二进制代码。这被称为即时编译（JIT）。即时编译会增加应用程序加载时间，但允许应用程序受益于每次新设备驱动带来的编译器改进。它还使应用程序能够在编译时不存在的设备上运行。

当设备驱动程序为应用程序即时编译PTX代码时，会自动缓存生成的二进制代码副本，以避免在后续调用应用中重复编译。该缓存称为计算缓存，在设备驱动升级时会自动失效，以便应用程序受益于设备驱动内置的即时编译器的改进。

自CUDA最早版本以来，PTX在运行时编译JIT的方式和时间有所放宽，允许在何时以及是否编译部分或全部内核时提供更多灵活性。"懒加载"部分描述了可用的选项以及如何控制JIT行为。还有一些环境变量控制即时编译行为，如CUDA环境变量中所述。

作为使用nvcc编译CUDA C++设备代码的替代方案，NVRTC可以在运行时将CUDA C++设备代码编译为PTX。NVRTC是CUDA C++的一个运行时编译库；更多信息可见NVRTC用户指南。
